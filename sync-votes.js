...
async function getHouseVoteCount(year) {
  // Binary search for highest roll number since no index page is available
  try {
    let lo = 1, hi = 800;
    // First check if hi exists, if not shrink
    while (hi > lo) {
      const padded = String(hi).padStart(3, '0');
      const r = await fetch(`https://clerk.house.gov/evs/${year}/roll${padded}.xml`, { method: 'HEAD' });
      if (r.ok) break;
      hi = Math.floor(hi * 0.7);
    }
    if (hi <= 1) return 0;
    // Binary search between lo and hi
    while (lo < hi) {
      const mid = Math.floor((lo + hi + 1) / 2);
      const padded = String(mid).padStart(3, '0');
      const r = await fetch(`https://clerk.house.gov/evs/${year}/roll${padded}.xml`, { method: 'HEAD' });
