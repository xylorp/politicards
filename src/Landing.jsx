...
export default function Landing({ nav }) {
  ...
  useEffect(() => {
    if (!demoQuery.trim()) { setDemoMembers([]); return; }
    const q   = demoQuery.toLowerCase().trim();
    const alt = NICKNAMES[q] ?? null;
    fetch('/data/members.json')
      .then(r => r.json())
      .then(all => setDemoMembers(
        all.filter(m => {
          const name = (m.name ?? '').toLowerCase();
          return name.includes(q) || (alt && name.includes(alt));
        })
      ))
      .catch(() => {});
  }, [demoQuery]);
    ...
    <div style={{ minHeight: '100vh', background: '#1a1510', fontFamily: 'DM Sans, sans-serif', overflowX: 'hidden' }}>
      ...
      <nav style={{
        ...
        <div
          ...
          style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 24, color: '#c9a84c', letterSpacing: 4, cursor: 'pointer', marginBottom: '0.5rem' }}
        >
          POLITICARDS
        </div>
        <div style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'center' }}>
          <span className="nav-link" onClick={nav.toSearch} style={{ fontSize: 12, color: '#8a7a5a', cursor: 'pointer', transition: 'color 0.2s' }}>Search</span>
          <span className="nav-link" onClick={nav.toBrowse} style={{ fontSize: 12, color: '#8a7a5a', cursor: 'pointer', transition: 'color 0.2s' }}>Browse All</span>
          <span
            className="cta-primary"
            onClick={nav.toSearch}
            style={{ fontSize: 12, fontWeight: 500, color: '#1a1510', background: '#c9a84c', padding: '6px 14px', borderRadius: 6, cursor: 'pointer', transition: 'all 0.2s' }}
          >
            Find your rep
          </span>
        </div>
      ...
      <div style={{
        ...
        {/* Background texture */}
        <div style={{ position: 'absolute', inset: 0, opacity: 0.04, backgroundImage: 'repeating-linear-gradient(45deg,white 0px,white 1px,transparent 0px,transparent 50%)', backgroundSize: '28px 28px', pointerEvents: 'none' }} />
        ...
        <div style={{ maxWidth: 1400, margin: '0 auto', width: '100%', display: 'flex', alignItems: 'flex-start', gap: '3rem', flexWrap: 'wrap', justifyContent: 'center' }}>

          {/* Left — copy */}
          <div style={{ flex: '1 1 300px', minWidth: 0, maxWidth: 540 }}>
            ...
            {/* Stats row */}
            <div className="fade-up delay-4" style={{ display: 'flex', gap: 20, marginTop: 48, flexWrap: 'wrap' }}>
              {STATS.map(({ val, lbl }) => (
                <div key={lbl}>
                  <div style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 24, color: '#c9a84c', letterSpacing: 2, lineHeight: 1 }}>{val}</div>
          ...
          <div className="fade-up delay-2" style={{ flex: '2 1 700px', minWidth: 300, display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ position: 'relative' }}>
              <input
                type="text"
                value={demoQuery}
                onChange={e => setDemoQuery(e.target.value)}
                placeholder="Try a name… (e.g. Bernie, Collins, Smith)"
                style={{ width: '100%', padding: '13px 16px 13px 44px', background: '#2a2018', border: '1px solid #4a3e28', borderRadius: 10, color: '#f0ece0', fontSize: 14, fontFamily: 'DM Sans, sans-serif', outline: 'none', boxSizing: 'border-box', transition: 'border-color 0.2s' }}
                onFocus={e => e.target.style.borderColor = '#c9a84c'}
                onBlur={e => e.target.style.borderColor = '#4a3e28'}
              />
              <svg style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', opacity: 0.5 }} width="16" height="16" viewBox="0 0 16 16" fill="none">
                <circle cx="7" cy="7" r="5" stroke="#c9a84c" strokeWidth="1.5"/>
                <line x1="11" y1="11" x2="15" y2="15" stroke="#c9a84c" strokeWidth="1.5" strokeLinecap="round"/>
              </svg>
            </div>
            {demoQuery.trim() && (
              <div style={{ fontSize: 12, color: '#5a4e38', letterSpacing: 0.5 }}>
                {demoMembers.length === 0 ? 'No matches found' : `${demoMembers.length} member${demoMembers.length !== 1 ? 's' : ''} found`}
              </div>
            )}
            {!demoQuery.trim() && (
              <div style={{ fontSize: 12, color: '#3a3028', letterSpacing: 0.5, textAlign: 'center', padding: '2rem 0' }}>
                Search any name to see real cards
              </div>
            )}
            <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: '1.5rem', justifyContent: 'flex-start', alignItems: 'flex-start' }}>
              {demoMembers.slice(0, 4).map(m => (
                <div key={m.bioguideId} onClick={() => nav.toDetail(m.bioguideId)}
                  style={{ cursor: 'pointer', filter: 'drop-shadow(0 8px 24px rgba(0,0,0,0.4))', transition: 'transform 0.2s', flexShrink: 0, width: 340 }}
              ...
              {demoMembers.length > 4 && (
                <div style={{ width: '100%', textAlign: 'center', fontSize: 12, color: '#5a4e38' }}>
                  +{demoMembers.length - 4} more — <span onClick={nav.toSearch} style={{ color: '#c9a84c', cursor: 'pointer' }}>search to see all →</span>
                </div>
              )}
            </div>
          </div>
